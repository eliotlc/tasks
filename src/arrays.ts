...
export const shoutIfExclaiming = (messages: string[]): string[] => {
    return messages
        .map((str: string): string => {
            return str.at(-1) === "!" ? str.toUpperCase() : str;
        })
...
export function countShortWords(words: string[]): number {
    return words.reduce((accumulated: number, str: string): number => {
        return accumulated + (str.length < 4 ? 1 : 0);
    }, 0);
}
...
export function allRGB(colors: string[]): boolean {
    return colors.every((color: string): boolean => {
        return ["red", "blue", "green"].indexOf(color) >= 0;
    });
}
...
export function makeMath(addends: number[]): string {
    return addends.length === 0 ?
            "0=0"
        :   `${addends.reduce((currentTotal: number, num: number) => currentTotal + num, 0)}=${addends.join("+")}`;
}
